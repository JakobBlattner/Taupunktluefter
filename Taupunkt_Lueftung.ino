...
void setup() {
  ...
  pinMode(ALWAYS_ON_IN, INPUT);
  pinMode(ALWAYS_ON_OUT, OUTPUT);
  digitalWrite(ALWAYS_ON_OUT, HIGH);
...
void loop() {
  ...
  bool fan_always_on = !digitalRead(ALWAYS_ON_IN);

  Serial.print("fan_always_on is ");
  Serial.println(fan_always_on);
  Serial.println();

  if(fan_always_on == false){
    ...
    delay(8000); // Zeit um das Display zu lesen
    wdt_reset(); // Watchdog zurücksetzen
    ...
    while(millis() - startTime < interval){
      lcd.setCursor(0,2);
      lcd.print(F("Naechste Messung:"));
      lcd.setCursor(0,3);
      lcd.print(String(interval/1000 - ((millis() - startTime)/1000)) + " Sek.");
      delay(100);
    }
    wdt_reset();   // Watchdog zurücksetzen 
  }
  else{ //activating relais, if fan switch is on "always on"
    digitalWrite(RELAIPIN, RELAIS_EIN); // Relais einschalten
    lcd.clear();
    lcd.print(F("Lueftung IMMER AN"));
